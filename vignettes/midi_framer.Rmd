...
We can create an R6 object of class `"MidiFramer"` from a midi file path by passing it to the  
...
The field `mf` is a `miditapyr.MidiFrames()` object. After its first element `mfr$mf$midi_file`, a [mido midi file object](https://mido.readthedocs.io/en/latest/midi_files.html) with the raw data, there are the 3 dataframes:
...
And you can write the midi file resulting of the `MidiFrames` object back to disk:
